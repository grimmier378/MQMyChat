...
int MyChatDatabase::GetSchemaVersion()
{
...
void MyChatDatabase::SetSchemaVersion(int version)
{
    sqlite3_stmt* stmt = nullptr;
...
void MyChatDatabase::RunMigrations()
{
    int currentVersion = GetSchemaVersion();

    if (currentVersion < 1)
    {
        ExecSQL("DROP TABLE IF EXISTS char_channel_overrides");

        ExecSQL(
            "CREATE TABLE IF NOT EXISTS char_channel_overrides ("
            "id INTEGER PRIMARY KEY AUTOINCREMENT, "
            "char_name TEXT NOT NULL, server TEXT NOT NULL DEFAULT '', "
            "preset_id INTEGER NOT NULL, channel_id INTEGER NOT NULL, "
            "enabled INTEGER NOT NULL DEFAULT 1, echo TEXT NOT NULL DEFAULT '/say', "
            ...
            "UNIQUE(char_name, server, preset_id, channel_id), "
...
void MyChatDatabase::InitSchema()
{
    ...
    ExecSQL("CREATE INDEX IF NOT EXISTS idx_char_channel_overrides_lookup ON char_channel_overrides(char_name, server, preset_id)");

    sqlite3_stmt* tplStmt = nullptr;
    if (PrepareAndStep("SELECT id FROM presets WHERE id = -1", tplStmt))
    {
        bool exists = (sqlite3_step(tplStmt) == SQLITE_ROW);
        sqlite3_finalize(tplStmt);
        if (!exists)
        {
            ExecSQL("INSERT INTO presets (id, preset_name, server, created_by) VALUES (-1, '_template_', '', '_system_')");
            SeedDefaultChannels(-1);
        }
    }
}
...
void MyChatDatabase::GetPresetList(const std::string& server, std::vector<PresetInfo>& outList)
{
    ...
    sqlite3_stmt* stmt = nullptr;
    if (!PrepareAndStep("SELECT id, preset_name, server, created_by FROM presets WHERE server=? AND id > 0", stmt))
        return;
...
void MyChatDatabase::LoadCharChannelOverrides(const std::string& server, const std::string& charName,
    int presetId, MyChatSettings& outSettings)
{
    sqlite3_stmt* stmt = nullptr;
    if (!PrepareAndStep(
        "SELECT channel_id, enabled, echo, main_enable, enable_links, "
        "pop_out, locked, scale, main_font_size, tab_order "
        "FROM char_channel_overrides WHERE char_name=? AND server=? AND preset_id=?", stmt))
        return;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        auto it = outSettings.channels.find(channelId);
        if (it == outSettings.channels.end())
            continue;

        auto& ch = it->second;
        ch.enabled     = sqlite3_column_int(stmt, 1) != 0;
        const char* echoText = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
        ch.echo        = echoText ? echoText : "/say";
        ch.mainEnable  = sqlite3_column_int(stmt, 3) != 0;
        ch.enableLinks = sqlite3_column_int(stmt, 4) != 0;
        ch.popOut      = sqlite3_column_int(stmt, 5) != 0;
        ch.locked      = sqlite3_column_int(stmt, 6) != 0;
        ch.scale       = static_cast<float>(sqlite3_column_double(stmt, 7));
        ch.fontSize    = sqlite3_column_int(stmt, 8);
        ch.tabOrder    = sqlite3_column_int(stmt, 9);
    }
    ...
    int presetId, const MyChatSettings& settings)
{
    ...
    if (PrepareAndStep(
        "DELETE FROM char_channel_overrides WHERE char_name=? AND server=? AND preset_id=?", delStmt))
    {
    ...
    for (const auto& [channelId, chan] : settings.channels)
    {
        ...
        sqlite3_bind_int(stmt, 5, chan.enabled ? 1 : 0);
        sqlite3_bind_text(stmt, 6, chan.echo.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(stmt, 7, chan.mainEnable ? 1 : 0);
        sqlite3_bind_int(stmt, 8, chan.enableLinks ? 1 : 0);
        sqlite3_bind_int(stmt, 9, chan.popOut ? 1 : 0);
        sqlite3_bind_int(stmt, 10, chan.locked ? 1 : 0);
...
void MyChatDatabase::SeedDefaultChannels(int presetId)
{
    ...
    run(fmt::format("INSERT INTO channels (preset_id, channel_id, name, enabled, echo, main_enable, enable_links, pop_out, locked, scale, main_font_size, tab_order) VALUES ({}, 6, 'Guild', 1, '/gu', 1, 0, 0, 0, 1.0, 16, 11)", presetId));
    ...
    run(fmt::format("INSERT INTO channels (preset_id, channel_id, name, enabled, echo, main_enable, enable_links, pop_out, locked, scale, main_font_size, tab_order) VALUES ({}, 12, 'Raid', 1, '/rsay', 1, 0, 0, 0, 1.0, 16, 12)", presetId));
...
int MyChatDatabase::CreateBlankPreset(const std::string& server, const std::string& charName, const std::string& name)
{
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "INSERT INTO presets (preset_name, server, created_by) VALUES (?, ?, ?)";
    if (sqlite3_prepare_v2(m_db, sql, -1, &stmt, nullptr) != SQLITE_OK)
    {
        WriteChatf("\ar[MyChat] Failed to prepare preset insert: %s", sqlite3_errmsg(m_db));
        return -1;
    }
    ...
    MyChatSettings templateSettings;
    LoadSettings(-1, templateSettings);
    if (!templateSettings.channels.empty())
        SaveSettings(newId, templateSettings);
    else
        SeedDefaultChannels(newId);
